...
void main() {
  ...
  test('test 3: API function takes longer than timeout set', () async {
    ...
    // Defining behaviour: throws an error on calling
    Future<AuthClient> mockClientViaUserConsent(ClientId clientId, List<String> scope, Function launchFunction()) async {
      await Future.delayed(new Duration(milliseconds: 200));
      return mockAuthClient;
    }

    // Expected behaviour
    ResourcePicker resourcePicker = new ResourcePicker.test("client_id", "client_secret", "enterprise_id", "redirect_url", mockClientViaUserConsent, resourcePickerTimeoutDuration: new Duration(milliseconds: 100));
    expect(resourcePicker.resourcePickerURL, "https://accounts.google.com/o/oauth2/auth?client_id=client_id&redirect_uri=redirect_url&response_type=code&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fsdm.service&state=state");
    expect(await resourcePicker.askForAuthorization(), "authorization failed");
  });
